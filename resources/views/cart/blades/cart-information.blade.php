<div class="w-full lg:w-1/4 lg:sticky top-5 flex flex-col gap-y-4">
    ...
    @php
        $isDisabled = !empty($cartItems->toArray())
    @endphp
    <a
        @if(!$isDisabled)
            href="{{ route('payment.index') }}"
        @endif
        class="w-full mt-4 flex items-center gap-x-1 justify-center
        @if($isDisabled) disabled @else  bg-blue-500 text-white hover:bg-blue-600 @endif
